...
import {
  ...
  FiFilter, FiXCircle, FiMapPin, FiChevronRight, FiChevronDown,
  FiArrowRight, FiList
} from "react-icons/fi";
...
// Create custom icons for weeks of supply
const createWeeksSupplyIcon = (store, selectedCategory) => {
  const { critical_count, low_count, adequate_count, high_count } = store;
  ...
  if (selectedCategory && selectedCategory !== 'all') {
    switch (selectedCategory) {
      ...
      case 'Low':
        ...
        break;
      case 'Adequate':
        severityClass = 'severity-adequate';
        count = adequate_count;
        break;
      case 'High':
        ...
        break;
      default:
        severityClass = 'severity-all';
        count = store.total_skus;
    }
  } else {
    // All stores view - show critical count as indicator
    if (critical_count > 0) {
      severityClass = 'severity-critical';
      count = critical_count;
    } else if (low_count > 0) {
...
function Dashboard() {
  ...
  // Fetch weeks of supply store summary
  const fetchWeeksSupplySummary = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/weeks-of-supply/store-summary`,
        { headers }
      );

      if (response.data.success) {
        console.log('Weeks supply stores:', response.data.data);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [dashboardRes, storesRes, alertsRes, availabilityRes] = await Promise.allSettled([
          axios.get(`${BASE_URL}/dashboard`, { headers }),
          axios.get(`${BASE_URL}/stores`, { headers }),
          axios.get(`${BASE_URL}/alerts`, { headers }),
          axios.get(`${BASE_URL}/availability`, { headers }),
        ...
        // Fetch weeks of supply data after main data is loaded
  ...
  // Combine store data with coordinates
  const storesWithCoords = useMemo(() => {
    return weeksSupplyStores.map(store => {
      ...
      return {
        ...
        store_name: locationData?.store_name || `Store ${store.store_id}`,
  ...
  // Calculate store counts for each category
  const storeCounts = useMemo(() => {
    return {
      'all': storesWithCoords.length,
      'Critical': storesWithCoords.filter(store => store.critical_count > 0).length,
      'Low': storesWithCoords.filter(store => store.low_count > 0).length,
      'Adequate': storesWithCoords.filter(store => store.adequate_count > 0).length,
      'High': storesWithCoords.filter(store => store.high_count > 0).length,
    };
  }, [storesWithCoords]);

  const handleStoreClick = (store, category = null) => {
    setSelectedStore(store);
    setSkuDetailsCategory(category);
    setShowSKUDetails(true);
  };

  const handleCategoryClickFromTooltip = (category) => {
  ...
  const handleCloseSKUDetails = () => {
    ...
    setSkuDetailsCategory(null);
  };

  const handleViewAllSKUs = (store) => {
    setSelectedStore(store);
    setSkuDetailsCategory('all');
    setShowSKUDetails(true);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-slate-900 text-white font-sans flex relative">
      ...
      {/* Sidebar */}
      <div className={`bg-slate-800 border-r border-slate-700 shadow-lg transition-all duration-300 ${sidebarOpen ? "w-64 p-6" : "w-0 p-0 overflow-hidden"} flex flex-col`}>
        {sidebarOpen && (
          <>
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-white text-xl font-bold">Control</h2>
              <button onClick={() => setSidebarOpen(false)} className="text-slate-400 hover:text-white"><FiX size={24} /></button>
            </div>
        ...
        <header className="relative flex justify-center items-center mb-6">
          <div className="text-center">
            ...
            <p className="text-sm text-slate-400 mt-1">
              Weeks of Supply Monitoring - {storeCounts[selectedCategory]} stores showing
            </p>
          </div>
          <button 
            ...
            <div className="lg:col-span-2 space-y-6 flex flex-col">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-slate-800 rounded-lg p-4 shadow-lg border border-slate-700 flex items-center space-x-4">
                  <div className="bg-slate-900 p-3 rounded-full"><FiTrendingUp size={24} className="text-blue-400" /></div>
                  <div>
                    <p className="text-sm font-medium text-slate-400">Current Demand</p>
                    <p className="text-2xl font-bold text-white">{data.metrics.current_demand.toLocaleString()}</p>
                  </div>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-slate-800 rounded-lg p-4 shadow-lg border border-slate-700 flex items-center space-x-4">
                  <div className="bg-slate-900 p-3 rounded-full"><FiBox size={24} className="text-green-400" /></div>
                  <div>
                    <p className="text-sm font-medium text-slate-400">Inventory Position</p>
                    <p className="text-2xl font-bold text-white">{data.metrics.inventory_position.toLocaleString()}</p>
                  </div>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-slate-800 rounded-lg p-4 shadow-lg border border-slate-700 flex items-center space-x-4">
                  <div className="bg-slate-900 p-3 rounded-full"><FiCalendar size={24} className="text-yellow-400" /></div>
                  <div>
                    <p className="text-sm font-medium text-slate-400">Weeks Of Supply</p>
                    <p className="text-2xl font-bold text-white">{data.metrics.weeks_of_supply.toLocaleString()}</p>
                  </div>
                </motion.div>
              </div>

              {/* Network View Map with Weeks of Supply */}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-slate-800 rounded-lg p-4 shadow-lg border border-slate-700 flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-4 flex-shrink-0">
                  <p className="font-bold text-white text-lg">Weeks of Supply - Network View</p>
                  <div className="text-xs flex space-x-2">
                    <span className="bg-red-900/50 px-2 py-1 rounded text-red-300">
                      {storeCounts.Critical} Critical
                    </span>
                  ...
                  >
                    ...
                    {filteredStores.map((store) => (
                      <AutoPanMarker 
                        ...
                        icon={createWeeksSupplyIcon(store, selectedCategory)}
                      >
                        <Popup 
                          closeButton={true}
                          autoClose={false}
                          closeOnClick={false}
                          className="custom-interactive-popup"
                        >
                          {showSKUDetails && selectedStore?.store_id === store.store_id ? (
                            <SKUDetailsPopup 
                              storeId={store.store_id}
                              category={skuDetailsCategory}
                            ...
                            <StoreSummaryTooltip 
                              store={store}
                              onCategoryClick={handleCategoryClickFromTooltip}
                              onViewAllSKUs={() => handleViewAllSKUs(store)}
                            />
                          )}
                        </Popup>
                      </AutoPanMarker>
