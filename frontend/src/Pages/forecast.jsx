...
import { FiBarChart2, FiCpu, FiClock, FiAlertCircle, FiTrendingUp, FiTrendingDown, FiTarget, FiInfo, FiPlayCircle } from 'react-icons/fi';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from "react-router-dom";
// ✨ REMOVED: No longer importing the separate AccuracyDashboard
import Chatbot from '../components/Chatbot';
...
const ForecastPage = () => {
    ...
    const [loading, setLoading] = useState({
        ...
        accuracy: false,
        logs: false,
        run: false, // For the new "Run Forecast" button
    });
    const [error, setError] = useState(null);
    ...
    // ✨ NEW: Function to trigger a new forecast run
    const handleRunForecast = async () => {
        const token = getToken();
        if (!token) {
            setError({ message: "Authentication required to run a forecast." });
            return;
        }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.error || 'Failed to start forecast run.');
            }
            
            // On success, switch to the logs tab and refresh the logs
            setActiveTab('logs');
            fetchLogs();
    ...
    const fetchForecastData = useCallback(async (currentFilterType, value) => {
        ...
        const body = {
            past_weeks: weeksToShow,
            future_weeks: weeksToShow,
        };
    ...
    useEffect(() => {
        if (!isInitialized || weeksToShow < 1) return;

        let value = null;
        if (filterType === 'store') value = selectedStore;
        if (filterType === 'sku') value = selectedSku;

        if (filterType === 'all' || value) {
            fetchForecastData(filterType, value);
        } else {
             setChartData([]); // Clear chart if a filter type is selected but no item is chosen
        }

    }, [isInitialized, filterType, selectedStore, selectedSku, weeksToShow, fetchForecastData]);
    
    // ✨ NEW: Dynamic questions for the chatbot based on the active tab
    const chatbotQuestions = useMemo(() => {
        ...
        switch(activeTab) {
            case 'accuracy': return accuracyQuestions;
            case 'logs': return logsQuestions;
            case 'forecast':
            default: return forecastQuestions;
        }
    }, [activeTab]);
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 bg-slate-900 text-slate-300 min-h-screen font-sans">
            <div className="max-w-7xl mx-auto">
                ...
                <main>
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'accuracy' && (
                            <motion.div key="accuracy" variants={panelVariants} initial="hidden" animate="visible" exit="exit">
                                {/* ✨ MODIFIED: Using the self-contained AccuracyPanel now */}
                                <AccuracyPanel 
            ...
            {/* ✨ MODIFIED: Passing dynamic questions to the chatbot */}
            <Chatbot questions={chatbotQuestions} />
        </div>
    );
};

// --- Child Components (all existing child components remain the same) ---
